...
class ProfileController extends Controller {
	...
	public function index(User $user) {
        return Theme::view('profile::form',array(
			'page_title' => Lang::get('action.my profile'),
	...
	public function password(User $user) {
        return Theme::view('profile::form-password',array(
			'page_title' => Lang::get('action.change password'),
            'user' =>  $user->find(Auth::user()->id)
        ));
    }
	...
	public function do_update_profile() {
        ...
        if($validate->fails()) {
			...
			if(!empty($user_id)) {
				...
				$user->save();
				$message = Lang::get('profile::message.update successfully');
			} 
			//params json
			$params ['success'] =  true;
	...
	public function do_update_password() {
        ...
        if($validate->fails()) {
			...
			if(!empty($user_id)) {
				...
				$user->save();
				$message = Lang::get('profile::message.change password successfully');
			} 
			//params json
			$params ['success'] =  true;
			$params ['message'] =  $message;			
